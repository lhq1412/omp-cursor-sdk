...
- Unset `PI_CURSOR_SETTING_SOURCES` omits SDK ambient settings (`settingSources` is not passed to `Agent.create`). Opt in with `PI_CURSOR_SETTING_SOURCES=all` or a comma list such as `user,project`.
