...
- `src/cursor-sdk-runtime.ts` owns the lazy `@cursor/sdk` import and side-effect-free runtime prewarm promise; `src/cursor-sdk-runtime-prewarm.ts` schedules that prewarm on local `cursor-sdk` `session_start` / `before_agent_start` after session runtime restore, without awaiting it.
