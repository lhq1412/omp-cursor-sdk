...
Options:
  ...
  --context-windows <file>      Optional JSON file with {"contextWindows": {"model": 123}}
                                ...
                                ~/.omp/agent/cursor-sdk-context-windows.json after live runs.
...
Notes:
  ...
  - Context-window keys are limited to canonical SDK base IDs or catalog context
    evidence such as model@272k. SDK aliases and stale identities are omitted.`);
}
...
function formatContextWindows(models, checkpointWindows, fallbackContextWindow) {
	...
	return `// Generated from Cursor SDK checkpoint tokenDetails.maxTokens on ${date}.\n// Refresh with: npm run refresh:cursor-snapshots -- --write --context-windows ~/.omp/agent/cursor-sdk-context-windows.json\n// Keys are canonical SDK base IDs or catalog context evidence such as model@272k.\n// Two-tier contexts retain separate evidence keys even when their OMP model identity\n// converges. SDK aliases and stale IDs are omitted. Values are observed or conservative\n// default/non-Max-mode limits and may override a catalog context label when the\n// completed SDK checkpoint reports a different effective limit.\nexport const BUNDLED_CONTEXT_WINDOWS = {\n${lines.join("\n")}\n} as const satisfies Record<string, number>;\n`;
}
