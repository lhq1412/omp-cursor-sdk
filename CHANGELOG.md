...
- Park local Cursor SDK `customTools.read` on the existing OMP bridge pending map so OMP's builtin `read` loop owns execution and UI; other builtins stay on nested `CursorExecHandlers`. Cloud is unchanged.
