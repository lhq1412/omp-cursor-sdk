...
- Canonicalize local session-agent pool-key inputs and hash the bridge MCP wire contract (name, composed description including prompt guidelines, schema) so provenance-only metadata no longer recreates the SDK agent.
- Prewarm `@cursor/sdk` in the background after local `cursor-sdk` session restore without issuing a model request.
- Record TTFT phase markers on the provider debug sink (`turn_start` through `first_omp_stream_event` from the pi stream tap, with `agent_acquired` at the final local acquire) so remaining latency can be attributed to SDK/backend work.
