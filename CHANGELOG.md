...
- Persist local SDK transcript message offsets in v3 resume handles and read them synchronously before `Agent.send()`, so resumed turns no longer block on `Agent.messages.list()` recounts; legacy handles without a watermark skip one turn of transcript WebSearch/WebFetch replay, then rebuild the watermark after a successful run.
