...
- Record TTFT phase markers on the provider debug sink (`turn_start` through `first_omp_stream_event`, with `agent_acquired` at the final local acquire) so remaining latency can be attributed to SDK/backend work.
